                    ...
                    TrkrDefs::TrkrId::tpcId, TrkrDefs::TrkrId::micromegasId})
  {
    for (const auto& hitsetkey : clusters->getHitSetKeys(det))
    {
      ...
      for (auto iter = range.first; iter != range.second; ++iter)
      {
        ...
        Acts::Vector3 glob;
        m_scluskey=key; 
      //  std::cout << "Cluster key: " << m_scluskey << " Type m_scluskeys "<<typeid(m_scluskey).name()<< std::endl;
        // NOT IMPLEMENTED YET
       ...
       ++hitsetiter)
  {
         ...
         ++hitr)
    {
      ...
      case TrkrDefs::TrkrId::tpcId:
      {
        ...
        //std::cout<<"layer "<<m_hitlayer<<" m_hitpad "<<m_hitpad<<" m_side "<<m_side<<std::endl;
                                           ...
                                           PHCompositeNode* topNode)
{
 ...
 // std::cout << "Cluster Key from Track: " << ckey << " Type m_cluskeys(vec) "<<typeid(m_cluskeys).name()<<" type ckey "<<typeid(ckey).name()<<std::endl;
/*
  ...
  for (const auto& [key, track] : *trackmap)
  {
    ...
    if (!m_doAlignment)
    {
      for (const auto& ckey : get_cluster_keys(track))
      {
        ...
        //fillClusterBranchesKF(clusterhitassocmap,ckey, track, global_raw, topNode);
      }
    }
