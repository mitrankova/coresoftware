#include "Full_PolyClusterResiduals.h"
...
namespace
{
  ...
  HelixCircle make_track_circle(const Full_PolyTrack* trk,
  ...
  bool line_xy_at_z(const Full_PolyTrack* trk,
                    const double z,
                    const double arc_direction,
                    double& x_state,
                    double& y_state)
  ...
  bool line_z_at_radius(const Full_PolyTrack* trk,
                        const double target_r,
                        const double reference_z,
                        const double arc_direction,
                        double& z_state)
  ...
  bool line_z_at_dca_to_vertex(const Full_PolyTrack* trk,
                               const double vertex_x,
                               const double vertex_y,
                               const double arc_direction,
                               double& z_at_dca,
  ...
  bool project_track_to_z(const Full_PolyTrack* trk,
                          const double z,
                          const double magnetic_field_tesla,
                          const double arc_direction,
  ...
  double cluster_line_residual2(const Full_PolyTrack* poly_track,
...
Full_PolyClusterResiduals::Full_PolyClusterResiduals(const std::string& name,
                                                   const std::string& outfilename)
  : SubsysReco(name)
  , m_outfilename(outfilename)
  , m_clusterNodeName("TPC_POLYCLUSTERS")
  , m_fullTrackNodeName("REFIT_FULL_POLYTRACKS")
  , m_trackVertexNodeName("TPC_POLYTRACKVERTICES")
{
}

Full_PolyClusterResiduals::~Full_PolyClusterResiduals()
{
  if (m_outfile)
  {
    delete m_outfile;
...
int Full_PolyClusterResiduals::Init(PHCompositeNode* /*unused*/)
{
  ...
  m_tree = new TTree("residuals", "TPC poly cluster residuals from refitted full poly tracks");
  m_tree->Branch("event", &m_event, "event/i");
  m_tree->Branch("full_track_id", &m_fullTrackId, "full_track_id/i");
  m_tree->Branch("tpc_poly_track_id", &m_tpcPolyTrackId, "tpc_poly_track_id/i");
  ...
  m_tree->Branch("nsilicon_clusters", &m_nsiliconClusters, "nsilicon_clusters/i");
  ...
  m_tree->Branch("score", &m_score, "score/D");
...
bool Full_PolyClusterResiduals::get_nodes(PHCompositeNode* topNode)
{
  ...
  m_trackVertices = findNode::getClass<Tpc_PolyTrackVertexContainer>(topNode, m_trackVertexNodeName);
  if (!m_trackVertices && Verbosity() > 0)
  {
    std::cerr << Name() << " - missing " << m_trackVertexNodeName
...
void Full_PolyClusterResiduals::reset_tree_values()
{
  m_event = m_evt;
  m_fullTrackId = 0;
  m_tpcPolyTrackId = 0;
  ...
  m_score = std::numeric_limits<double>::quiet_NaN();
...
int Full_PolyClusterResiduals::process_event(PHCompositeNode* topNode)
{
  ++m_evt;
  ...
  if (m_trackVertices)
  {
    for (unsigned int ivtx = 0; ivtx < m_trackVertices->size(); ++ivtx)
    {
      const Tpc_PolyTrackVertex* vtx = m_trackVertices->get_vertex(ivtx);
      if (!vtx)
      {
        continue;
      }
  ...
  const unsigned int npoly_tracks = m_fullTracks->size();
  for (unsigned int ifinal = 0; ifinal < npoly_tracks; ++ifinal)
  {
    const Full_PolyTrack* poly_track = m_fullTracks->get_track(ifinal);
    ...
    const double score = poly_track->get_score();

    if (use_straight_line)
    {
      if (choose_collision_vertex_line(m_trackVertices, poly_track, arc_direction, vertex_x, vertex_y, vertex_z, rdca))
      {
        double z_at_zero = 0.0;
        line_z_at_dca_to_vertex(poly_track, 0.0, 0.0, arc_direction, z_at_zero, rdca_zero);
      }
    }
    else if (choose_collision_vertex(m_trackVertices, circle, vertex_x, vertex_y, vertex_z))
    {
    ...
    reset_tree_values();
    m_event = m_evt;
    m_fullTrackId = poly_track->get_track_id();
    m_tpcPolyTrackId = poly_track->get_tpc_poly_track_id();
    ...
    m_nsiliconClusters = poly_track->get_n_silicon_clusters();
...
int Full_PolyClusterResiduals::End(PHCompositeNode* /*unused*/)
{
  if (m_outfile)
  {
    m_outfile->cd();
